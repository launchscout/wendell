...
module.exports = (robot) => {
  robot.respond(/pizza me/i, msg => msg.send(msg.random(pizzas)))

  robot.respond(/pizza bomb/i, function (msg) {
    const randomSize = msg.random([2, 5, 8, 10, 15])
    const sortedPizzas = pizzas.sort(() => 0.5 * Math.random()).slice(1, randomSize)
    return Array.from(sortedPizzas).map((pizza) =>
      msg.send(pizza))
  })
}
