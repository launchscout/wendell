...
module.exports = function (robot) {
  robot.respond(/catbug me/i, msg => msg.send(msg.random(catbugs)))

  robot.respond(/catbug bomb/i, function (msg) {
    const randomSize = msg.random([2, 5, 8, 10, 15])
    const sortedCatbugs = catbugs.sort(() => 0.5 * Math.random()).slice(1, randomSize)
    return Array.from(sortedCatbugs).map((catbug) =>
      msg.send(catbug))
  })
}
